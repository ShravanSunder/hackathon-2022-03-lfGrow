...
library ProfileTokenURILogic {
   ...
   */
  function _getSVGImageBase64Encoded(string memory handleWithAtSymbol, string memory imageURI) internal pure returns (string memory) {
    return
      Base64.encode(
        abi.encodePacked(
  ...
  function _getSVGProfilePicture(string memory imageURI) internal pure returns (string memory) {
    if (_shouldUseCustomPicture(imageURI)) {
      return string(abi.encodePacked('<image id="custom-picture" preserveAspectRatio="xMidYMid slice" height="450" width="450" href="', imageURI, '"/>'));
    } else {
      return
  ...
  function _handleLengthToFontSize(uint256 handleLength) internal pure returns (uint256) {
    return handleLength <= MAX_HANDLE_LENGTH_WITH_DEFAULT_FONT_SIZE ? DEFAULT_FONT_SIZE : DEFAULT_FONT_SIZE - (handleLength - 12) / 2;
  }
