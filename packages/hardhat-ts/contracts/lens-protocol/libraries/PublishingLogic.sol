...
library PublishingLogic {
  ...
  function createProfile(
    ...
    bytes memory followModuleReturnData = _initFollowModule(profileId, vars.followModule, vars.followModuleData, _followModuleWhitelisted);
  ...
  function setFollowModule(
    ...
    bytes memory followModuleReturnData = _initFollowModule(profileId, followModule, followModuleData, _followModuleWhitelisted);
  ...
  function createPost(
    ...
    emit Events.PostCreated(profileId, pubId, contentURI, collectModule, collectModuleReturnData, referenceModule, referenceModuleReturnData, block.timestamp);
  }
  ...
  function createComment(
    ...
    if (refModule != address(0)) {
      IReferenceModule(refModule).processComment(vars.profileId, vars.profileIdPointed, vars.pubIdPointed);
    }
  ...
  function createMirror(
    ...
    (uint256 rootProfileIdPointed, uint256 rootPubIdPointed, ) = Helpers.getPointedIfMirror(profileIdPointed, pubIdPointed, _pubByIdByProfile);
    ...
    if (refModule != address(0)) {
      IReferenceModule(refModule).processMirror(profileId, rootProfileIdPointed, rootPubIdPointed);
    }

    emit Events.MirrorCreated(profileId, pubId, rootProfileIdPointed, rootPubIdPointed, referenceModule, referenceModuleReturnData, block.timestamp);
  }
  ...
  function _initPubCollectModule(
    ...
    return ICollectModule(collectModule).initializePublicationCollectModule(profileId, pubId, collectModuleData);
  }
  ...
  function _initPubReferenceModule(
    ...
    if (referenceModule != address(0)) {
      ...
      return IReferenceModule(referenceModule).initializeReferenceModule(profileId, pubId, referenceModuleData);
    } else {
      return new bytes(0);
    }
  }
  ...
  function _initFollowModule(
    ...
    if (followModule != address(0)) {
      ...
      bytes memory returnData = IFollowModule(followModule).initializeFollowModule(profileId, followModuleData);
      return returnData;
    } else {
      return new bytes(0);
    }
  ...
  function _validateHandle(string calldata handle) private pure {
    ...
    for (uint256 i = 0; i < byteHandle.length; ++i) {
      if ((byteHandle[i] < "0" || byteHandle[i] > "z" || (byteHandle[i] > "9" && byteHandle[i] < "a")) && byteHandle[i] != ".")
