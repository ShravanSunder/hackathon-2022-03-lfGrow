...
contract LimitedTimedFeeCollectModule is ICollectModule, FeeModuleBase, FollowValidationModuleBase {
  ...
  function initializePublicationCollectModule(
    ...
    (uint256 collectLimit, uint256 amount, address currency, address recipient, uint16 referralFee) = abi.decode(
      data,
      (uint256, uint256, address, address, uint16)
    );
    if (collectLimit == 0 || !_currencyWhitelisted(currency) || recipient == address(0) || referralFee > BPS_MAX || amount < BPS_MAX)
  ...
  function processCollect(
    ...
    if (_dataByPublicationByProfile[profileId][pubId].currentCollects >= _dataByPublicationByProfile[profileId][pubId].collectLimit) {
