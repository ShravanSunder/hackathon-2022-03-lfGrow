...
contract TimedFeeCollectModule is ICollectModule, FeeModuleBase, FollowValidationModuleBase {
  ...
  function initializePublicationCollectModule(
    ...
    (uint256 amount, address currency, address recipient, uint16 referralFee) = abi.decode(data, (uint256, address, address, uint16));
    if (!_currencyWhitelisted(currency) || recipient == address(0) || referralFee > BPS_MAX || amount < BPS_MAX) revert Errors.InitParamsInvalid();
